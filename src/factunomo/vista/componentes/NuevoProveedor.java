...
public class NuevoProveedor extends FormularioBase{
	private static final long serialVersionUID = 1L;
	  ...
	  */
	 public NuevoProveedor(JDialog principal){
	 ...
	 private void creaVentana(){
		 // Nueva ventana
		 ventana = new JDialog(principal,"Nuevo Proveedor",true);
		 ...
		 idProveedor.setText(modelo.nuevoIdProveedor());
	 ...
	 class GuardarAL implements ActionListener {
		 public void actionPerformed(ActionEvent evento){
				...
				try {
					...
					int result = JOptionPane.showConfirmDialog(ventana,
							"El ID proveedor ha de ser un número entero", "¡Advertencia!",
							JOptionPane.DEFAULT_OPTION);
					idProveedor.requestFocus();
					return;
				}
				if (modelo.nombreProveedorDuplicado(nombre.getText())){
					int result = JOptionPane.showConfirmDialog(ventana,
				...
				modelo.guardarProveedor(proveedor);
				ventana.dispose();
			}
	 }
