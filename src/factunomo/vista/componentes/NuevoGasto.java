...
import factunomo.vista.componentes.NuevoIngreso.cambioCliente;
...
public class NuevoGasto extends FormularioBase{
	 ...
	 */
	public NuevoGasto(ConfiguracionVO datos, JFrame principal){
		...
		this.gastoEditar = new GastoVO();
		this.gastoEditar.setIdGasto(modelo.nuevoIdGasto(datosUsuario.getEjercicio()));
		this.listaDetallesGasto = new ArrayList<DetalleGastoVO>();
	    ...
	    this.datosProveedor = new ProveedorVO();
	    this.datosProveedor.setIdFormaPago("01");
	...
	private void creaVentana(){
		...
		ventana = new JDialog(principal,"Nuevo Gasto",true);
		...
		String idFP=datosProveedor.getIdFormaPago();
		if (idFP.equals("")) idFP="01";
		datosFormaPago=modelo.getFormaPago(idFP);
		gastoEditar.setIdFormaPago(idFP);
		...
		// Total
		IRPF.setText(Decimales(gastoEditar.getTipoIRPF()));
		visualizarTotal();
	...
	private void visualizarFormaPago () {
		...
		cfor.gridy++;
		this.numeroCuenta.setText(this.gastoEditar.getNumeroCuenta());
	...
	private void visualizarTotal(){
		...
		ctot.gridx++;
		cadena= " ";
		if (gastoEditar.getDescuento()!=null){
		...
		ctot.gridx++;
		cadena="" + Decimales(gastoEditar.getTipoIRPF());
		ctot.anchor = GridBagConstraints.LINE_END;
	...
	private void actualizarTotal(){
		...
		if (gastoEditar.getDescuento()<0 || gastoEditar.getDescuento()>100) err=true;
		if (err){
		...
		if (gastoEditar.getTipoIRPF()<0 || gastoEditar.getTipoIRPF()>100) err=true;
		if (err){
			int result = JOptionPane.showConfirmDialog(ventana,
	...
	private void actualizarComboProveedor()
	{
		...
		if (datosProveedor.getConIRPF()) gastoEditar.setTipoIRPF(datosUsuario.getIRPF());
	...
	class GuardarAL implements ActionListener {
		public void actionPerformed(ActionEvent evento){
			...
			gastoEditar.setDescuento(Float.parseFloat(cadena));
			try {
				modelo.guardarGasto(gastoEditar,listaDetallesGasto);
			} catch (Exception e) {
			}			
			cancelado=true;
			ventana.dispose();
