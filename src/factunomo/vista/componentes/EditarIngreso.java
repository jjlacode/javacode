...
import factunomo.vista.componentes.NuevoIngreso.CuentaAL;
import factunomo.vista.componentes.NuevoIngreso.cambioFormaPago;
...
public class EditarIngreso extends FormularioBase{
	 ...
	 */
	public EditarIngreso(IngresoVO ingresoEditar, ArrayList<DetalleIngresoVO> listaDetallesIngreso, ConfiguracionVO datos, JFrame principal){
		...
		this.ingresoEditar = ingresoEditar;
		this.listaDetallesIngreso = listaDetallesIngreso;
		...
		this.datosCliente = modelo.getCliente(this.ingresoEditar.getNombre());
		this.datosFormaPago = modelo.getFormaPago(this.ingresoEditar.getIdFormaPago());
		this.datosUsuario = datos;
		// Campos formulario
		this.fecha = new JDateChooser();
		this.fecha.setDate(ingresoEditar.getFecha());
	...
	private void creaVentana(){
		Float DescuentoInicial=ingresoEditar.getDescuento();
		Float IRPFinicial=ingresoEditar.getTipoIRPF();
		String CuentaInicial=ingresoEditar.getNumeroCuenta();
		String PagoInicial=modelo.getFormaPago(ingresoEditar.getIdFormaPago()).getNombre();
		// Nueva ventana
		
		ventana = new JDialog(principal,"Editar Ingreso",true);
		...
		// Inicializar
		this.cliente.setSelectedItem(this.ingresoEditar.getNombre());
		...
		this.ingresoEditar.setTipoIRPF(IRPFinicial);	
		actualizarTotal();
	...
	private void visualizarCliente () {
		...
		bcliente.addActionListener(new ClienteAL());
		panelCliente.add(bcliente,cpro);
		cpro.gridx = 0;
		cpro.gridy++;
		...
		cpro.gridx = 0;
		cpro.gridy++;		
		panelCliente.add(new JLabel("CIF:"),cpro);		
		cpro.gridx++;
	...
	private void visualizarTotal(){
		...
		ctot.gridx++;
		cadena= " ";
		if (ingresoEditar.getDescuento()!=0f){
		...
		cadena =""+ Decimales(subTotal);
		ctot.weightx = 1;
		if (ingresoEditar.getTipoIRPF()!=0f)	cadena="-"+cadena;
		panelTotal.add(new JLabel(cadena),ctot);
		ctot.gridx++;
		base = base - subTotal;
		cadena = "" + Decimales(base);
	...
	/**
	 * Añadir Escucha combo clientees raton
	 * @param box
	 */
	private void addClienteRatonListener(JComboBox<String> box) {
		try {
	 ...
	 * @param box
	 */
	private void addFormaPagoRatonListener(JComboBox<String> box) {
		try {
			Field popupInBasicComboBoxUI = BasicComboBoxUI.class.getDeclaredField("popup");
			popupInBasicComboBoxUI.setAccessible(true);
			BasicComboPopup popup = (BasicComboPopup) popupInBasicComboBoxUI.get(box.getUI());
	...
	class GuardarAL implements ActionListener {
		public void actionPerformed(ActionEvent evento){
			...
			if (!ingresoEditar.getIdIngreso().equals(idIngreso.getText()) && modelo.idGastoDuplicado(idIngreso.getText())){
				int result = JOptionPane.showConfirmDialog(ventana,
			...
			try {
				modelo.actualizarIngreso(ingresoEditar,listaDetallesIngreso);
			} catch (Exception e) {
			}
