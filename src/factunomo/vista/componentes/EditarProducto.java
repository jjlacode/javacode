...
public class EditarProducto extends FormularioBase{
	  ...
	  */
	 public EditarProducto(JDialog principal, ProductoVO producto){
	 ...
	 private void creaVentana(){
		 ...
		 codigo.setText(producto.getIdProducto());
		 ...
		 ct.gridy++;
		 nombre.requestFocus();
		 precio.setText(Decimales(producto.getPrecio()));
		 panelDetalle.add(new JLabel("Precio:"),ct);
		 ct.gridx++;
		 panelDetalle.add(precio,ct);
		 ct.gridx = 0;
		 ct.gridy++;
		 IVA.setText(Decimales(producto.getIVA()));
	 ...
	 class GuardarAL implements ActionListener {
		 public void actionPerformed(ActionEvent evento){
			 	...
			 	IVA.setText(IVA.getText().replace(",","."));
				...
				if (!codigo.getText().equals(producto.getIdProducto())&&modelo.idProductoDuplicado(codigo.getText())){
					int result = JOptionPane.showConfirmDialog(ventana,
